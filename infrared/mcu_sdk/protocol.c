...
{
  {DPID_DATA_SEND, DP_TYPE_STRING},
  {DPID_DATA_RECV, DP_TYPE_STRING},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_string_update(DPID_DATA_RECV,当前红外接收指针,当前红外接收数据长度); //STRING型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_data_send_handle
功能描述 : 针对DPID_DATA_SEND的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_data_send_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret,i;

    //STRING类型数据处理
		q_mian_buf[Q_Main.rear].length=length;
		for(i=0;i<length;i++)
		{
			q_mian_buf[Q_Main.rear].data[i]=value[i];
		}
    rear_inc(&Q_Main);
    F_TASK_InfraRed++;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_DATA_SEND,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_DATA_SEND:
            //红外发送处理函数
            ret = dp_download_data_send_handle(value,length);
        break;
